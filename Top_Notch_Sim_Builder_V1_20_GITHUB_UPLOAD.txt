...
(function(){
  ...
  const VERSION = "2026-09-25-consolidated-v1.20";
  ...
  function init(){
    ...
    const monitors = [
      ...
      {id:"43-4k-touch",name:'43" 4K 60Hz Touchscreen Monitor',price:1499.99,size:'43"',resolution:"3840 × 2160 4K",refresh:"60Hz",role:"touch4k",tier:3,notes:"Touchscreen · LED LCD display · 10-bit / 1.07B colors · up to 350 cd/m² brightness · 5000:1 contrast · 178° viewing angle · 50,000-hour backlight lifespan"}
    ];
    ...
    function monitorFit(mon){
      ...
      if(projector.resolution==="4K"){
        ...
        return {status:"yellow",label:"Not 4K",text:"This monitor can still be used, but it will not display 4K. Choose the 4K monitor for a true 4K monitor display."};
      }
...
@media(max-width:560px){#tnsb-root .tnsb-radio-grid{grid-template-columns:1fr}#tnsb-root .tnsb-submit-area{align-items:stretch;flex-direction:column}#tnsb-root .tnsb-card h3{padding-right:0;margin-top:26px}#tnsb-root .tnsb-badge{left:12px;right:auto;max-width:calc(100% - 24px)}}
    ...
    function renderProjectors(){
      ...
      $("tnsb-projector-list").innerHTML=projectors.map(p=>{
        ...
        return `<div class="tnsb-card ${selected?"tnsb-selected":""}">${badge("neutral",`${Number(p.lumens).toLocaleString("en-CA")} Lumens`)}<button type="button" class="tnsb-card-main" data-projector-id="${p.id}"><h3>${esc(p.name)}</h3><div class="tnsb-price">${money(p.price)}</div><div class="tnsb-meta">${p.sku?`SKU ${esc(p.sku)}<br>`:""}${esc(p.notes)}<br>Throw ratio ${p.throw[0].toFixed(2)}–${p.throw[1].toFixed(2)}${range}</div></button>${detailLink(p.url)}</div>`;
      }).join("");
