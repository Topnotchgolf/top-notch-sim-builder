...
(function(){
  ...
  function init(){
    ...
    const defaultState = {
      ...
      puttingTurf:"none",roughEnabled:false,roughSqft:0,
    ...
    function puttingInfo(){
      if(state.puttingTurf==="none") return {price:0,length:0,material:"None",name:"No Putting Turf"};
      const r=room();
      const length=Math.max(10,Math.ceil(r.d||10));
      if(state.puttingTurf==="green") return {price:1550+Math.max(0,length-10)*150,length,material:`15' W × ${length}' L`,name:"NP45 Putting Green Turf – Green"};
      return {price:1850+Math.max(0,length-10)*185,length,material:`15' W × ${length}' L`,name:"NP45 Putting Green Turf – Black"};
    }

    function roughRecommendedSqft(){
      const r=room();
      return Math.max(0,(r.w-15)*r.d);
    }
    function roughPrice(){ return state.roughEnabled ? Math.max(0,Number(state.roughSqft)||0)*9 : 0; }
...
@media(max-width:560px){#tnsb-root .tnsb-radio-grid{grid-template-columns:1fr}#tnsb-root .tnsb-submit-area{align-items:stretch;flex-direction:column}#tnsb-root .tnsb-card h3{padding-right:0;margin-top:26px}#tnsb-root .tnsb-badge{left:12px;right:auto;max-width:calc(100% - 24px)}}
    ...
    ROOT.innerHTML=`
      <div class="tnsb-shell">
        ...
        <section class="tnsb-section" data-step="4">
          ...
          <div class="tnsb-inline-controls"><div class="tnsb-field"><label>Mat Width (ft)</label><input id="tnsb-mat-w" type="number" min="1" step="0.1" value="${esc(state.matW)}"></div><div class="tnsb-field"><label>Mat Length (ft)</label><input id="tnsb-mat-l" type="number" min="1" step="0.1" value="${esc(state.matL)}"></div></div>
        ...
        <section class="tnsb-section" data-step="5">
          ...
          <div class="tnsb-subsection"><h3>Putting Turf</h3><p>NP45 is supplied in a fixed 15 ft width. The builder prices the required roll length from the room depth, with a 10 ft minimum.</p><div id="tnsb-putting-list" class="tnsb-choice-grid"></div><div id="tnsb-putting-calc" class="tnsb-calc"></div></div>
          <div class="tnsb-subsection"><h3>Optional Rough Turf</h3><p>Rough turf is a separate optional purchase at $9.00/sq ft. If the room is wider than 15 ft, the builder can suggest coverage for the width outside the NP45 putting turf.</p><div class="tnsb-option-line"><input id="tnsb-rough-enabled" type="checkbox"><div><strong>Add Rough Turf</strong><span>Field / Olive · 3/4 in pile height · 15 ft untrimmed roll width.<br><strong style="color:var(--tn-accent)">Recommended to cover up any empty floor space.</strong></span></div></div><div class="tnsb-inline-controls"><div class="tnsb-field"><label>Rough Turf Coverage (sq ft)</label><input id="tnsb-rough-sqft" type="number" min="0" step="1" value="${esc(state.roughSqft)}"></div><div class="tnsb-field"><label>Recommended From Room</label><div class="tnsb-calc" style="margin:0"><span id="tnsb-rough-recommended"></span> <button id="tnsb-use-rough-rec" class="tnsb-mini-btn" type="button">Use Recommended</button></div></div></div><div id="tnsb-rough-calc" class="tnsb-calc"></div>${detailLink(URLS.rough)}</div>
    ...
    function renderFlooring(){
      ...
      const puttingOptions=[
        ...
        {id:"green",name:"NP45 Putting Green Turf – Green",price:p.name.includes("Green")?p.price:null,meta:"Nylon · 3/8 in pile height · 15 ft roll width · Forest/Olive"},
        {id:"black",name:"NP45 Putting Green Turf – Black",price:p.name.includes("Black")?p.price:null,meta:"Nylon · 3/8 in pile height · 15 ft roll width · Black"}
      ];
      $("tnsb-putting-list").innerHTML=puttingOptions.map(o=>{
        const testState=state.puttingTurf; let price=0;
        if(o.id==="green"){ const r=room(),len=Math.max(10,Math.ceil(r.d||10)); price=1550+Math.max(0,len-10)*150; }
        if(o.id==="black"){ const r=room(),len=Math.max(10,Math.ceil(r.d||10)); price=1850+Math.max(0,len-10)*185; }
        return `<div class="tnsb-card ${testState===o.id?"tnsb-selected":""}"><button type="button" class="tnsb-card-main" data-putting="${o.id}"><h3>${esc(o.name)}</h3><div class="tnsb-price">${money(price)}</div><div class="tnsb-meta">${esc(o.meta)}</div></button>${o.id==="none"?"":detailLink(URLS.np45)}</div>`;
      }).join("");
      const r=room();
      $("tnsb-putting-calc").innerHTML=state.puttingTurf==="none"?"No putting turf selected.":`Finished room coverage: <strong>${round1(r.w)}' W × ${round1(r.d)}' D = ${round1(r.w*r.d)} sq ft</strong><br>Material supplied: <strong>${esc(p.material)}</strong> · Estimated putting turf price: <strong>${money(p.price)}</strong><br><span style="color:#93a098">Turf is supplied in a fixed 15 ft width and cut to fit during installation. Some excess material may remain.</span>`;
      $("tnsb-rough-enabled").checked=!!state.roughEnabled;
      $("tnsb-rough-sqft").disabled=!state.roughEnabled;
      const rec=roughRecommendedSqft();
      $("tnsb-rough-recommended").innerHTML=rec>0?`<strong>${round1(rec)} sq ft</strong> to fill width beyond 15 ft`:`<strong>0 sq ft</strong> automatically required`;
    ...
    function renderReview(){
      ...
      const floorParts=[];
      if(state.puttingTurf!=="none") floorParts.push(`${put.name} — ${money(put.price)}`);
    ...
    async function submitQuote(){
      ...
      const payload={
        ...
        "Putting Turf":state.puttingTurf==="none"?"None":`${put.name} — ${put.material} — ${money(put.price)} CAD`,
        "Rough Turf":state.roughEnabled?`${round1(state.roughSqft)} sq ft — ${money(roughPrice())} CAD`:"None",
        "Flooring Base":state.floorBase==="none"?"None":`${base.name} — ${base.detail} — ${money(base.price)} CAD`,
    ...
    ROOT.addEventListener("click",e=>{
      ...
      const fm=e.target.closest("[data-front-mode]"); if(fm){state.frontMode=fm.dataset.frontMode;save();frontFilterOptions();renderFront();renderProjectors();updateTotals();return;}
      const enc=e.target.closest("[data-enclosure-key]"); if(enc&&!enc.disabled){state.enclosureKey=decodeURIComponent(enc.dataset.enclosureKey);save();renderFront();renderProjectors();updateTotals();return;}
