...
(function(){
  ...
  const VERSION = "2026-09-24-consolidated-v1.12";
  ...
  function init(){
    ...
    const mats = [
      ...
      {id:"hotshot-softer",name:"Carl’s HotShot Golf Mat System Softer",pricing:"fixed",url:URLS.hotshot,pricesUsd:{"4x5":645.95,"5x8":915.90,"4x9":929.95,"6x10":1209.95}},
      {id:"hotshot-harder",name:"Carl’s HotShot Golf Mat System Harder",pricing:"fixed",url:URLS.hotshot,pricesUsd:{"4x5":699.95,"5x8":965.90,"4x9":979.95,"6x10":1259.95}}
    ];
    ...
    const defaultState = {
      ...
      matId:null,matSizeId:null,matW:5,matL:7,
    ...
    function matSelectionText(){
      ...
      if(m.pricing==="fixed"){
        const s=selectedMatSize();
        return s&&matSizeAllowed(s)?`${s.label} · ${money(matPrice())}`:"Select a fixed HotShot size";
      }
      const d=matDimensions();
...
@media(max-width:560px){#tnsb-root .tnsb-radio-grid{grid-template-columns:1fr}#tnsb-root .tnsb-submit-area{align-items:stretch;flex-direction:column}#tnsb-root .tnsb-card h3{padding-right:0;margin-top:26px}#tnsb-root .tnsb-badge{left:12px;right:auto;max-width:calc(100% - 24px)}}
    ...
    ROOT.innerHTML=`
      <div class="tnsb-shell">
        ...
        <section class="tnsb-section" data-step="0">
          <div class="tnsb-kicker">STEP 1 OF 9</div><h2>Room Dimensions</h2>
        ...
        <section class="tnsb-section" data-step="1">
          <div class="tnsb-kicker">STEP 2 OF 9</div><h2>Enclosure + Screen</h2>
        ...
        <section class="tnsb-section" data-step="2">
          <div class="tnsb-kicker">STEP 3 OF 9</div><h2>Launch Monitor</h2>
        ...
        <section class="tnsb-section" data-step="3">
          <div class="tnsb-kicker">STEP 4 OF 9</div><h2>Projector</h2>
        ...
        <section class="tnsb-section" data-step="4">
          <div class="tnsb-kicker">STEP 5 OF 9</div><h2>Hitting Mat</h2>
          <p class="tnsb-lead">Choose your hitting surface. EZ-Tee is priced by custom coverage area. Carl’s HotShot mats are sold only in the fixed sizes shown below.</p>
          ...
          <div id="tnsb-mat-custom-controls" class="tnsb-inline-controls"><div class="tnsb-field"><label>Mat Width (ft)</label><input id="tnsb-mat-w" type="text" inputmode="decimal" value="${esc(state.matW)}"></div><div class="tnsb-field"><label>Mat Length (ft)</label><input id="tnsb-mat-l" type="text" inputmode="decimal" value="${esc(state.matL)}"></div></div>
        ...
        <section class="tnsb-section" data-step="5">
          <div class="tnsb-kicker">STEP 6 OF 9</div><h2>Flooring / Turf</h2>
          ...
          <div class="tnsb-btnrow"><button class="tnsb-btn" data-go="4">← Back</button><button class="tnsb-btn tnsb-primary" data-go="6">Computer + Monitor →</button></div>
        </section>

        <section class="tnsb-section" data-step="6">
          <div class="tnsb-kicker">STEP 7 OF 9</div><h2>Computer + Monitor</h2>
          <p class="tnsb-lead">Choose the computer and display together in one step. Computer recommendations respond to the selected launch monitor and projector resolution, while monitor recommendations update from the selected gaming tower.</p>
          <div class="tnsb-subsection" style="margin-top:0;padding-top:0;border-top:0"><h3>Computer</h3><div id="tnsb-computer-list" class="tnsb-choice-grid"></div></div>
          ...
          <div class="tnsb-subsection"><h3>Monitor</h3><p>Choose a gaming display or touchscreen control monitor.</p><div id="tnsb-monitor-list" class="tnsb-choice-grid"></div></div>
          <div class="tnsb-btnrow"><button class="tnsb-btn" data-go="5">← Back</button><button class="tnsb-btn tnsb-primary" data-go="7">Accessories →</button></div>
        </section>

        <section class="tnsb-section" data-step="7">
          <div class="tnsb-kicker">STEP 8 OF 9</div><h2>Accessories</h2>
          ...
          <div class="tnsb-btnrow"><button class="tnsb-btn" data-go="6">← Back</button><button class="tnsb-btn tnsb-primary" data-go="8">Review →</button></div>
        </section>

        <section class="tnsb-section" data-step="8">
          <div class="tnsb-kicker">STEP 9 OF 9</div><h2>Review + Request a Quote</h2>
          ...
          <div class="tnsb-btnrow"><button class="tnsb-btn" data-go="7">← Back</button><span></span></div>
        </section>
      </div>
    `;
    ...
    function goStep(index){
      state.step=clamp(Number(index)||0,0,8); save();
    ...
    function renderForStep(step){
      ...
      if(step===6){ renderComputers(); renderMonitors(); }
      if(step===7) renderAccessories();
    ...
    ROOT.addEventListener("click",e=>{
      ...
      const hand=e.target.closest("[data-handed]"); if(hand){state.handedness=hand.dataset.handed;if(state.handedness==="both"&&selectedMat()?.pricing==="fixed"&&selectedMatSize()?.oneSidedOnly)state.matSizeId=null;save();renderHandedness();renderMats();updateTotals();return;}
      const fm=e.target.closest("[data-front-mode]"); if(fm){state.frontMode=fm.dataset.frontMode;clearAdditionalFlooringForSimBooth();save();frontFilterOptions();renderFront();renderProjectors();if(state.step===4)renderMats();if(state.step===5)renderFlooring();updateTotals();return;}
      const enc=e.target.closest("[data-enclosure-key]"); if(enc&&!enc.disabled){state.enclosureKey=decodeURIComponent(enc.dataset.enclosureKey);clearAdditionalFlooringForSimBooth();save();renderFront();renderProjectors();if(state.step===4)renderMats();if(state.step===5)renderFlooring();updateTotals();return;}
      ...
      const mat=e.target.closest("[data-mat-id]"); if(mat){state.matId=mat.dataset.matId;if(selectedMat()?.pricing!=="fixed")state.matSizeId=null;if(state.handedness==="both"&&selectedMatSize()?.oneSidedOnly)state.matSizeId=null;save();renderMats();updateTotals();return;}
      const matSize=e.target.closest("[data-mat-size]"); if(matSize&&!matSize.disabled){const s=hotShotSizes.find(x=>x.id===matSize.dataset.matSize);if(matSizeAllowed(s)){state.matSizeId=s.id;save();renderMats();if(state.step===5)renderFlooring();updateTotals();}return;}
    ...
    ["tnsb-room-w","tnsb-room-d","tnsb-room-h"].forEach((id,i)=>$(id).addEventListener("input",e=>{state[["roomW","roomD","roomH"][i]]=Number(e.target.value)||0;save();if(state.step===1)renderFront();if(state.step===5)renderFlooring();if(state.step===7)renderAccessories();updateTotals();}));
